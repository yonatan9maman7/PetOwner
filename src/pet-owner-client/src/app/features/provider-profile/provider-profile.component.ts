              ...
              <div class="flex items-center gap-3">
                @if (profile()!.instagramUrl) {
                  <a [href]="profile()!.instagramUrl" target="_blank" rel="noopener noreferrer"
                     class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium
                            ...
                            hover:from-purple-600 hover:to-pink-600 transition-all shadow-sm">
                ...
                @if (profile()!.facebookUrl) {
                  <a [href]="profile()!.facebookUrl" target="_blank" rel="noopener noreferrer"
                     class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium
                            bg-[#1877F2] text-white hover:bg-[#166FE5] transition-all shadow-sm">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
...
export class ProviderProfileComponent implements OnInit {
  ...
  /** Public profile uses user id as providerId; hide self-chat CTA. */
  readonly showSendMessageCta = computed(() => {
    const p = this.profile();
    const uid = this.auth.userId();
    if (!p || !uid) return true;
    return p.providerId.toLowerCase() !== uid.toLowerCase();
  });
