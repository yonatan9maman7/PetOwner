...
public class PalsController : ControllerBase
{
        ...
        [FromQuery] string? size)
    {
        ...
        var raw = await query.Select(u => new
        {
            ...
            Pets = u.Pets.Select(p => new { p.Id, p.Name, Species = p.Species, p.Breed, p.Gender, p.BirthDate, p.Age, p.ImageUrl, p.DogSize, p.Sterilization, p.TagsCsv }).ToList()
        ...
        var results = raw
            ...
            .Select(x => new PalDto(
                ...
                null,
                x.u.Bio,
                x.u.Pets.Select(p => MapPet(p.Id, p.Name, p.Species.ToString(), p.Breed, p.Gender, p.BirthDate, p.Age, p.ImageUrl, p.DogSize, p.Sterilization, p.TagsCsv)).ToList(),
                x.u.LastActiveAt))
            .ToList();
    ...
    public async Task<IActionResult> StartBeacon([FromBody] CreateLiveBeaconDto dto)
    {
        ...
        var myPets = await _db.Pets.AsNoTracking()
            ...
            .Select(p => new { p.Id, p.Species, p.Name, p.Breed, p.Gender, p.BirthDate, p.Age, p.ImageUrl, p.DogSize, p.Sterilization, p.TagsCsv })
            .ToListAsync();
        if (myPets.Count == 0)
        ...
        var beaconPets = myPets.Where(p => requestedIds.Contains(p.Id))
            .Select(p => MapPet(p.Id, p.Name, p.Species.ToString(), p.Breed, p.Gender, p.BirthDate, p.Age, p.ImageUrl, p.DogSize, p.Sterilization, p.TagsCsv))
            .ToList();
        ...
        [FromQuery] string? species)
    {
        ...
        var raw = await _db.PlaydateBeacons.AsNoTracking()
            ...
            .Select(b => new
            {
                ...
                Pets = b.User.Pets.Select(p => new { p.Id, p.Name, Species = p.Species, p.Breed, p.Gender, p.BirthDate, p.Age, p.ImageUrl, p.DogSize, p.Sterilization, p.TagsCsv }).ToList()
            })
            .ToListAsync();
    ...
    private static PalPetDto MapPet(Guid id, string name, string species, string? breed, PetGender gender, DateTime? birthDate, int storedAge, string? imageUrl,
        DogSize? dogSize, SterilizationStatus sterilization, string tagsCsv) => new(
        id, name, species, breed, gender == PetGender.Unknown ? null : gender.ToString(), PetAgeHelper.CalculateAge(birthDate, storedAge), imageUrl,
        dogSize?.ToString(),
